...
async function main(): Promise<void> {
  ...
  if (cmd === "discover") {
    ...
    if (r.complementaryAdded > 0) console.log(`Thin niche → added ${r.complementaryAdded} adjacent-industry reference(s).`);
    for (const p of r.persisted) console.log(`  ✓ @${p.handle} — ${p.followers ?? "?"} followers · success ${p.successScore} · ${p.adjacency ? `adjacent:${p.adjacency}` : p.type === "local_intel" ? "local peer" : "reference"} (via ${p.source})`);
