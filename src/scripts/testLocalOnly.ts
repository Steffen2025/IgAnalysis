...
async function main() {
  ...
  console.log(`Inserted audit id=${audit.id} (mode=local_only)`);
  ...
  const bad = comps.filter((c) => c.competitor_type !== "local_intel");
  console.log(`\nCompetitors (${comps.length}, expecting all local_intel):`);
  ...
  if (bad.length > 0) {
    console.error(`❌ ${bad.length} competitors are not local_intel`);
    process.exit(1);
  }
  console.log(`✅ All competitors classified local_intel`);
  console.log(`Estimated cost: $${(result.itemsScraped * 0.0015).toFixed(4)}`);
}

main().catch((err) => {
  console.error("testLocalOnly failed:", err);
  process.exit(1);
});
